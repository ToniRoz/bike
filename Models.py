...
class QuantileEmbedding(nn.Module):
  def __init__(self, embedding_dim, output_dim):
    ...
    # Linear layer to project cosine embeddings to output dimension
...
class DQN(nn.Module):
  def __init__(self, args, action_space):
    ...
    # State feature extraction layers
    ...
    # Output layers (now output single values per action, not distributions)
  ...
  def forward(self, x, num_quantiles):
    """
    Args:
      x: State tensor of shape (batch_size, history_length * 1080)
    ...
    # Reshape to (batch_size, 1, hidden_size) and repeat for num_quantiles
    ...
    # Reshape back to (batch_size, num_quantiles, 1) and (batch_size, num_quantiles, action_space)
