...
class SequentialReplayBuffer():
  ...
  def _insert_vectorized(self, data: PyTree, env_mask: Optional[np.ndarray] = None) -> None:
      if env_mask is None:
          env_mask = np.ones(self.num_envs, dtype=bool)

      active_envs = np.where(env_mask)[0]  # indices of active environments

      def masked_set(x, y):
          x[self.current_ind[active_envs], active_envs] = y[active_envs]

      jax.tree.map(masked_set, self.data, data)

      # Update buffer state
      self.current_ind[active_envs] = (self.current_ind[active_envs] + 1) % self.capacity
      self.size[active_envs] = np.clip(self.size[active_envs] + 1, 0, self.capacity)
